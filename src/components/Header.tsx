...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-40 shadow-lg w-full overflow-hidden">
      {/* Top Header Bar: Logo & Main Actions */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center justify-between gap-3">
        {/* Brand & App Identifier */}
        <div className="flex items-center gap-3 cursor-pointer" onClick={onNewProject}>
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 via-indigo-500 to-cyan-400 p-0.5 shadow-md shadow-indigo-500/20 shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-extrabold text-white text-base tracking-tight">Meta-AI Builder</h1>
            ...
            <p className="text-xs text-slate-400 flex items-center gap-1.5">
              <span className="hidden sm:inline">Interactive AI Prompt Bridge</span>
              {projectName && (
                <>
                  <span className="text-slate-600 hidden sm:inline">•</span>
                  <span className="text-cyan-400 font-medium truncate max-w-[140px]">{projectName}</span>
                </>
              )}
            </p>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          {onOpenGitHubExport && (
            <button
              ...
              className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer shadow-sm"
          ...
          {onOpenHowItWorks && (
            <button
              ...
              className="px-2.5 py-1.5 bg-indigo-950/60 hover:bg-indigo-900/80 text-indigo-200 border border-indigo-500/30 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer shadow-sm"
            >
      ...
      {/* Bottom Sub-Bar: Pipeline Stage Stepper */}
      <div className="bg-slate-950/70 border-t border-slate-800/80 px-4 sm:px-6 lg:px-8 py-2 w-full overflow-x-auto min-w-0 scrollbar-none">
        <nav className="flex items-center gap-1 sm:gap-2 max-w-7xl mx-auto w-full min-w-max">
          {stages.map((st, idx) => {
              ...
              <React.Fragment key={st.id}>
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-[9px] opacity-70 leading-tight hidden md:block">{st.agent}</div>
                  </div>
                </button>
